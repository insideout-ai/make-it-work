...
[{{USER_NAME}} also keeps a live Open Loops canvas ({{OPEN_LOOPS_CANVAS_ID_OR_LINK}}) mirroring the memory file's open loops as checkboxes. If {{USER_NAME}} asks for an open-loops update mid-session, or checks something off and wants it reflected, update both the canvas and {{BRAIN_MD_FILENAME}} the same way the scheduled task does — don't wait for the next scheduled run.]
...
- The Open Loops canvas clause only applies if Part 3's optional canvas was actually set up. If not, remove it entirely rather than leaving a placeholder.
